...
Generate a **13-panel multi-figure illustration** (recommended: 4×4 grid with 3 cells for legend/title, or main 3×3 + supplementary 2×2) for an academic paper titled *"Towards Equitable AI in Pathology: A Systematic Review of Fairness Challenges and Debiasing Methods in Histopathology Imaging."* Each panel explains one of the **15 fairness metrics** from the paper's fairness-metric inventory (Table 1), using a **real published histopathology example** with actual numbers from the 78-study systematic review corpus. Some panels cover two closely related metrics (e.g., Panel 5: ECE + Subgroup Calibration Gap; Panel 9: Site-Classification Accuracy + Mutual Information), yielding 13 panels for 15 metrics. The figure serves as a visual glossary for histopathology researchers unfamiliar with fairness evaluation.
...
**Layout:** 4 columns × 4 rows (or 3×3 main + 2×2 supplementary) on a white/transparent background. Each panel is self-contained with a mini chart/diagram and a 1–2 line caption below it. Overall figure title at top: **"How Fairness Metrics Work — A Visual Guide with Real Examples from Histopathology AI"**
